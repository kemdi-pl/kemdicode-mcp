...
│   ├── audit-schedule-store.ts # Redis persistence for audit schedules
│   └── audit-scheduler.ts   # Self-organizing recurring audit engine
...
| Cluster Bus | 9 | `cluster-bus-status` `cluster-bus-topology` `cluster-bus-send` `cluster-bus-magistrale` `cluster-bus-flow` `cluster-bus-routing` `cluster-bus-inspect` `cluster-bus-file-read` `audit-scheduler` |
...
mcp:audit:*       — Audit schedules, history, and last-run timestamps
