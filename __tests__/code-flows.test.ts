...
describe('Code Flow Tests - KemdiCode MCP', () => {
  ...
  describe('6. Cognition & Memory Flows', () => {
    ...
    it('Ambient Learner: should gather silent knowledge', () => {
      const patterns = {
        toolUsage: ['file-read', 'file-write', 'grep'],
        successRate: 0.92,
  ...
  describe('9. Reinforcement Learning Flows', () => {
    it('Reward Logging: should log dopamine signals', () => {
      const rewards = [
        { type: 'success', value: 1.0, timestamp: Date.now() },
        { type: 'failure', value: -0.5, timestamp: Date.now() },
        { type: 'partial', value: 0.3, timestamp: Date.now() },
      ];
      expect(rewards.length).toBe(3);
    });

    it('Reward Stats: should aggregate reward statistics', () => {
      const stats = {
        totalRewards: 42,
        averageReward: 0.72,
        successRate: 0.85,
        topPattern: 'correct-confident',
      };
