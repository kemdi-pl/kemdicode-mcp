...
describe('Level 2 Integration: Tool Chain', () => {
  ...
  describe('I2.22: Tool schema validation consistency', () => {
    ...
    it('should validate required fields in ask-ai schema', () => {
      const schema = toolSchemas['ask-ai'];

      // Valid input
      const valid = schema.parse({
        prompt: '@src/test.ts',
      });
      expect(valid.prompt).toBe('@src/test.ts');
      ...
      // Invalid enum value
      expect(() => schema.parse({ prompt: '@test.ts', focus: 'invalid' })).toThrow();
    });

    it('should validate error-pattern schema with optional fields', () => {
      const schema = toolSchemas['error-pattern'];

      // Minimal valid input
      const minimal = schema.parse({
        pattern: '@src/buggy.ts',
        description: 'Button does not respond',
      });
      expect(minimal.pattern).toBe('@src/buggy.ts');
    ...
    it('should handle concurrent schema validations', async () => {
      ...
      const tasks = inputs.map((input, i) => ({
        name: `validate-${i}`,
        fn: async () => {
          const result = toolSchemas['ask-ai'].safeParse(input);
    ...
    it('should provide helpful error messages for invalid inputs', () => {
      const result = toolSchemas['ask-ai'].safeParse({
        focus: 'invalid-focus',
      });
      ...
      if (!result.success) {
        ...
        expect(issues.some((e) => e.path.includes('prompt'))).toBe(true);
      }
    });
  });
  ...
  describe('I2.23: Tool context sharing between executions', () => {
    it('should store context from tool execution', async () => {
      await toolExecutor.executeTool('ask-ai', { files: '@src/test.ts' });

      const history = await contextStorage.getSessionHistory(sessionId, 10);
      expect(history.length).toBe(1);
      expect(history[0].toolName).toBe('ask-ai');
    });

    it('should accumulate context from multiple tool executions', async () => {
      await toolExecutor.executeTool('ask-ai', { files: '@src/a.ts' });
      await toolExecutor.executeTool('semantic-search', { files: '@src/b.ts' });
      await toolExecutor.executeTool('error-pattern', {
        files: '@src/c.ts',
        description: 'Bug',
      });
      ...
      expect(toolNames).toContain('ask-ai');
      expect(toolNames).toContain('semantic-search');
      expect(toolNames).toContain('error-pattern');
    });

    it('should allow querying context by tool name', async () => {
      await toolExecutor.executeTool('ask-ai', { files: '@src/a.ts' });
      await toolExecutor.executeTool('ask-ai', { files: '@src/b.ts' });
      await toolExecutor.executeTool('semantic-search', { files: '@src/c.ts' });

      const askAiContext = await contextStorage.queryContext({
        sessionId,
        toolName: 'ask-ai',
        limit: 10,
      });

      expect(askAiContext.length).toBe(2);
      expect(askAiContext.every((c) => c.toolName === 'ask-ai')).toBe(true);
    });

    it('should preserve execution order in context', async () => {
      for (let i = 0; i < 5; i++) {
        await toolExecutor.executeTool('decision-journal', {
          files: `@src/file${i}.ts`,
          goal: 'readability',
        });
  ...
  describe('I2.24: Batch tool operations', () => {
    it('should execute multiple tools in parallel', async () => {
      const tools = [
        { name: 'ask-ai', args: { files: '@src/a.ts' } },
        { name: 'semantic-search', args: { files: '@src/b.ts' } },
    ...
    it('should execute multiple tools sequentially', async () => {
      ...
      expect(results[0].toolName).toBe('ask-ai');
      expect(results[1].toolName).toBe('semantic-search');
    ...
    it('should handle mixed tool types in batch', async () => {
      const tools = [
        { name: 'ask-ai', args: { files: '@src/test.ts', focus: 'security' } },
        { name: 'semantic-search', args: { files: '@src/test.ts', depth: 'deep' } },
        { name: 'error-pattern', args: { files: '@src/test.ts', description: 'Fix memory leak' } },
        { name: 'decision-journal', args: { files: '@src/test.ts', goal: 'performance' } },
  ...
  describe('I2.25: Tool error handling and recovery', () => {
    it('should handle individual tool failures gracefully', async () => {
      ...
      const result1 = await toolExecutor.executeTool('ask-ai', { files: '@src/test.ts' });
      expect(result1.success).toBe(true);

      // Failed execution
      await expect(
        toolExecutor.executeTool('ask-ai', { files: '@src/test.ts' }, { shouldFail: true })
      ).rejects.toThrow('Tool ask-ai failed');

      // Successful execution after failure (recovery)
      const result2 = await toolExecutor.executeTool('semantic-search', { files: '@src/test.ts' });
    ...
    it('should report detailed error information', async () => {
      const invalidInput = { focus: 'invalid' }; // Missing required 'prompt'

      const result = toolSchemas['ask-ai'].safeParse(invalidInput);
      expect(result.success).toBe(false);
      ...
      if (!result.success) {
        ...
        expect(errorMessages.some((e) => e.path === 'prompt')).toBe(true);
      }
    });
  ...
  describe('I2.26: Tool execution concurrency', () => {
    ...
    it('should maintain data integrity under concurrent load', async () => {
      const tasks = Array.from({ length: 30 }, (_, i) => ({
        name: `integrity-${i}`,
        fn: async () => {
          const result = await toolExecutor.executeTool('decision-journal', {
  ...
  describe('I2.27: Cross-tool data flow', () => {
    it('should allow tool output to inform next tool input', async () => {
      ...
      const _reviewResult = await toolExecutor.executeTool('ask-ai', {
      ...
      const fixResult = await toolExecutor.executeTool('error-pattern', {
        files: '@src/component.tsx',
    ...
    it('should support tool chain workflows', async () => {
      const workflow = [
        { name: 'semantic-search', args: { files: '@src/complex.ts', depth: 'deep' } },
        { name: 'ask-ai', args: { files: '@src/complex.ts', focus: 'quality' } },
        { name: 'decision-journal', args: { files: '@src/complex.ts', goal: 'readability' } },
      ];
    ...
    it('should aggregate context from multiple tool executions', async () => {
      ...
      await toolExecutor.executeTool('semantic-search', { files: '@src/services/user.ts' });
      await toolExecutor.executeTool('ask-ai', {
  ...
  describe('Complete tool chain workflow scenario', () => {
    it('should execute a full code improvement workflow', async () => {
      ...
      await toolExecutor.executeTool('semantic-search', {
        files: '@src/legacy-component.tsx',
        depth: 'deep',
        audience: 'senior',
      });
