...
**Example output:**
```
Based on your recent pattern (error-pattern → thinking-chain → ...)
Recommended next: write-tests (85% probability)
Alternative: self-critique (12% probability)

Top patterns:
1. error-pattern → thinking-chain → write-tests (used 23 times)
2. find-definition → find-references → semantic-search (used 15 times)
3. decision-journal → confidence-tracker → mental-model (used 12 times)
```
