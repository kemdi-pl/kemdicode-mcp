...
Fig.~\ref{fig:arch} shows the high-level system architecture. The server is organized into five principal layers: transport, session management, tool registry, bus architecture, and shared infrastructure (including Redis, LLM providers, and Tree-sitter).
...
Every message on the ClusterBus is a \code{ClusterSignal}. The principal fields are shown below (auxiliary fields such as \code{correlationId}, \code{metaTags}, and \code{schemaVersion} are omitted for brevity):
...
\begin{equation}
P_{\text{dup}}(s) = \begin{cases}
0 & \text{if } s \in R_{\text{recent}} \\
...
\noindent where $R_{\text{recent}}$ is the exact recent set and $B_{\text{actual}}$ is the set of IDs truly present in the bloom filter. This achieves zero false positives for the 2{,}000 most recent signals. To prevent saturation, the bloom filter is periodically reset after processing 15{,}000 signals; upon reset, only the current recent-set entries are re-inserted.

\subsubsection{HMAC Authentication}

Signals are authenticated using HMAC-SHA256. The signing process computes a signature over the complete JSON-serialized signal:

\begin{equation}
\text{HMAC} = H_K\bigl(\text{JSON.stringify}(s)\bigr)
\label{eq:hmac}
\end{equation}

\noindent where $H_K$ denotes HMAC-SHA256 with shared key $K$ and $s$ is the full signal object. Signing the entire payload rather than selected fields ensures that any tampering---including metadata modification---is detected. Verification uses a constant-time byte-by-byte XOR comparison loop to prevent timing side-channel attacks. Signals failing HMAC verification are silently dropped and logged.
...
\begin{description}[nosep,style=unboxed,leftmargin=0pt,font=\normalfont\itshape]
\item[Rate Limiting.] A sliding-window rate limiter with window size $W = 1$\,s and maximum capacity $C_{\max}$ signals per window. The effective throughput at time $t$ is bounded by:
\begin{equation}
\Theta(t) \leq C_{\max} \cdot \frac{1}{W} \quad \text{[signals/s]}
\label{eq:ratelimit}
\end{equation}
Excess signals are either buffered or dropped based on the configured backpressure policy.

\item[Circuit Breaker.] Implements the three-state pattern~\cite{nygard2007} modeled as a finite automaton $\mathcal{A} = (S, \Sigma, \delta, s_0)$ where $S = \{\text{closed}, \text{open}, \text{half-open}\}$, $s_0 = \text{closed}$, and the transition function $\delta$ is defined by:
\begin{equation}
\delta(s, \sigma) = \begin{cases}
\text{open}      & s = \text{closed} \wedge f \geq F_{\max} \\
\text{half-open} & s = \text{open} \wedge \Delta t \geq T_{\text{open}} \\
\text{closed}    & s = \text{half-open} \wedge p \geq P_{\min} \\
\text{open}      & s = \text{half-open} \wedge \sigma = \text{fail}
\end{cases}
\label{eq:circuit}
\end{equation}
where $f$ is the consecutive failure count ($F_{\max} = 10$), $\Delta t$ is the time since opening ($T_{\text{open}} = 5{,}000$\,ms), and $p$ is the successful probe count ($P_{\min} = 2$).
...
\item Stale detection: nodes exceeding the heartbeat timeout (default 15\,s) are marked stale and excluded from routing decisions.
...
The bus defines 12~typed channels organized into five domains, as shown in Table~\ref{tab:channels}.
...
\begin{definition}[Envelope Validity]
\label{def:envelope}
An envelope $E$ on channel $c$ is valid iff:
\begin{equation}
\mathcal{V}(E) \iff \mathcal{Z}_c(E.\text{payload}) \wedge E.\text{ttl} > 0 \wedge E.\text{ts} \leq t_{\text{now}}
\label{eq:validity}
\end{equation}
where $\mathcal{Z}_c$ is the Zod schema validator for channel $c$.
\end{definition}

\noindent Invalid envelopes cause synchronous rejection with descriptive error messages.
...
Handlers execute asynchronously via \code{queueMicrotask()} with configurable priority. Algorithm~\ref{alg:dispatch} formalizes the dispatch procedure. The chain depth limit of~8 prevents cascading event storms where handler~$A$ emits event~$e_1$, triggering handler~$B$ which emits~$e_2$, and so forth.

\begin{algorithm}[!t]
\caption{GlobalEventBus Dispatch}\label{alg:dispatch}
\SetAlgoLined
\SetKwInOut{Input}{Input}
\Input{event $e$, payload $\rho$, depth $d$}
$d \leftarrow d + 1$\;
\If{$d > D_{\max}$}{
  \textbf{log} warning, \textbf{return}\;
}
$H \leftarrow \text{handlers}[e.\text{type}]$\;
\textbf{sort} $H$ by priority descending\;
\ForEach{$h_i \in H$}{
  \textbf{queueMicrotask}$(h_i, \rho, d)$\;
}
$d \leftarrow d - 1$\;
\end{algorithm}

\noindent where $D_{\max} = 8$ is the maximum permitted chain depth.
...
For any signal $s$ entering the bus system, the total number of copies produced across all layers is bounded by:
\begin{equation}
...
\item[Quality-target.] Iterates pass $i$ until convergence. The termination condition is:
\begin{equation}
Q_i \geq Q_{\text{target}} \quad \vee \quad i \geq N_{\max}
\label{eq:passterm}
\end{equation}
...
Maintains calibrated confidence scores in the range $[0, 1]$ per domain. Calibration quality is measured by the Expected Calibration Error over a sliding window of $N$ predictions:
...
\noindent where predictions are partitioned into $B$ bins, $n_b$ is the bin count, $\text{acc}(b)$ is the observed accuracy, and $\text{conf}(b)$ is the mean predicted confidence. Trend data (improving, stable, declining) is derived from the ECE gradient over the window. Stored at \code{mcp:cognition:confidence:\{domain\}}.
...
\noindent where $S_r$ is the success rate, $C_t$ is task complexity (normalized), $A_s$ is average speed (inverse-normalized), and $C_o$ is collaboration score. To emphasize recent performance, scores undergo exponential temporal decay:

\begin{equation}
\text{Score}(t) = \text{Score}_0 \cdot \alpha^{\Delta t / \tau}
\label{eq:decay}
\end{equation}

\noindent where $\alpha = 0.95$ is the decay factor, $\Delta t$ is the elapsed time since the last activity, and $\tau = 1$\,day is the decay period. The tier mapping is defined as:

\begin{equation}
\text{Tier}(s) = \begin{cases}
\text{diamond}  & s \in [801, 1000] \\
\text{platinum} & s \in [601, 800] \\
\text{gold}     & s \in [401, 600] \\
\text{silver}   & s \in [201, 400] \\
\text{bronze}   & s \in [0, 200]
\end{cases}
\label{eq:tier}
\end{equation}
...
Shamir's Secret Sharing~\cite{shamir1979} enables distributed secret management. A secret $s$ is encoded as the constant term of a random polynomial of degree $t-1$ over a finite field $\mathbb{F}_p$:
...
\noindent where $a_1, \ldots, a_{t-1} \overset{\$}{\leftarrow} \mathbb{F}_p$. Each of the $n$ shares is a point $(i, f(i))$ for $i \in \{1, \ldots, n\}$. Reconstruction from any $t$ shares uses Lagrange interpolation to recover $f(0) = s$. The tools \code{mpc-split}, \code{mpc-distribute}, and \code{mpc-reconstruct} implement this protocol.
