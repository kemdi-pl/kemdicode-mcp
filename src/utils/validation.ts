...
/**
 * Safely parse JSON with type inference and fallback.
 * Prevents crashes from malformed JSON in Redis or external data.
 *
 * @param value - The string to parse (null/undefined returns fallback)
 * @param fallback - Value to return if parsing fails
 * @returns Parsed value or fallback
 *
 * @example
 * ```typescript
 * const data = safeJsonParse<User | null>(redisValue, null);
 * if (!data) return null;
 * ```
 */
export function safeJsonParse<T>(value: string | null | undefined, fallback: T): T {
  ...
  try {
    ...
    Logger.debug(`[safeJsonParse] Failed to parse JSON, returning fallback`);
    return fallback;
  }
}
