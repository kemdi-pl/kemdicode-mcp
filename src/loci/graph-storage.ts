...
export class GraphStorage extends RedisBackedService {
  ...
  /**
   * Batch-fetch edges by IDs using pipeline (eliminates N+1 queries).
   */
  private async batchGetEdges(edgeIds: string[]): Promise<GraphEdge[]> {
    if (!this.redis || edgeIds.length === 0) return [];

    const pipeline = this.redis.pipeline();
    for (const edgeId of edgeIds) {
      pipeline.hgetall(LOCI_KEYS.edge(edgeId));
    }
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      edges.push(this.parseEdge(data as Record<string, string>));
    }

    // Batch increment traversal counts
    if (edges.length > 0) {
      const incrPipeline = this.redis.pipeline();
      for (const edge of edges) {
        incrPipeline.hincrby(LOCI_KEYS.edge(edge.id), 'traversalCount', 1);
      }
      incrPipeline.exec().catch(() => {});
    }
  ...
  /**
   * Batch-fetch nodes by IDs using pipeline (eliminates N+1 queries).
   */
  private async batchGetNodes(nodeIds: string[]): Promise<GraphNode[]> {
    if (!this.redis || nodeIds.length === 0) return [];

    const pipeline = this.redis.pipeline();
    for (const nodeId of nodeIds) {
      pipeline.hgetall(LOCI_KEYS.node(nodeId));
    }
    const results = await pipeline.exec();
    if (!results) return [];
    ...
    for (let i = 0; i < results.length; i++) {
      const [err, data] = results[i];
      if (err || !data || typeof data !== 'object' || Object.keys(data as Record<string, string>).length === 0) continue;
      nodes.push(this.parseNode(data as Record<string, string>));
    }
    ...
    if (nodes.length > 0) {
      ...
      for (const node of nodes) {
        updatePipeline.hset(LOCI_KEYS.node(node.id), {
          lastAccessedAt: now,
          accessCount: node.accessCount + 1,
        });
      }
      updatePipeline.exec().catch(() => {});
    }
  ...
  async getOutgoingEdges(nodeId: string): Promise<GraphEdge[]> {
    ...
    try {
      ...
      return this.batchGetEdges(edgeIds);
    } catch (error) {
  ...
  async queryNodes(options: GraphQueryOptions): Promise<GraphNode[]> {
    ...
    try {
      ...
      // Start with type filter if specified (batched via pipeline)
      if (options.type) {
        const types = Array.isArray(options.type) ? options.type : [options.type];
        const pipeline = this.redis.pipeline();
        for (const type of types) {
          pipeline.smembers(LOCI_KEYS.nodesByType(type));
        }
        const results = await pipeline.exec();
        if (results) {
          for (const [err, ids] of results) {
            if (!err && Array.isArray(ids)) {
              nodeIds.push(...(ids as string[]));
            }
          }
        }
      ...
      // Load nodes (batched via pipeline)
      const allNodes = await this.batchGetNodes(nodeIds);
      const nodes: GraphNode[] = [];
  ...
  async getNeighbors(
    ...
    // Batch-fetch all neighbor nodes in one pipeline call
    return this.batchGetNodes(Array.from(neighborIds));
  }
