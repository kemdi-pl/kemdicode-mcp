...
import { VERSION, type ServerConfig, startHttpServer, stopHttpServer, broadcastNotification, createSessionServer } from './server/index.js';
...
async function main(): Promise<void> {
  ...
  const program = new Command()
    ...
    .option('--stdio', 'Run as stdio transport (for MCP client subprocess mode)')
    .parse();
  ...
  if (opts.context !== false) {
    ...
    if (!contextEnabled) {
      Logger.warn('Redis not available. Only basic tools (code analysis, file ops, git) will work.');
      Logger.warn('Features disabled without Redis: memory, kanban, cognition, agents, sessions, cluster bus.');
      Logger.warn('Install Redis: https://redis.io/docs/install/');
    }
  ...
  // Load AI config from Redis (persisted across restarts)
  try {
    const { loadConfigFromRedis } = await import('./tools/system/ai-config.tool.js');
    const loaded = await loadConfigFromRedis();
    if (loaded) {
      Logger.info('AI config restored from Redis');
      // Re-read serverConfig after Redis merge
      Object.assign(serverConfig, config.get('server'));
    }
  } catch {
    Logger.debug('Redis AI config load skipped (Redis not ready)');
  }
  ...
  if (opts.stdio) {
    // Stdio mode: run as subprocess for MCP clients (Claude Code, Cursor, etc.)
    // Logger already outputs to stderr (console.error/warn) so stdout stays clean for JSON-RPC.
    const transport = new StdioServerTransport();
    const server = createSessionServer('stdio');
    await server.connect(transport);

    Logger.info(`KemdiCode MCP v${VERSION} running in stdio mode`);

    // Eagerly load all lazy tool schemas so ListTools returns full schemas immediately.
    // Must complete before accepting requests — empty schemas break function calling.
    await warmupLazySchemas().catch((err) =>
      Logger.warn(`Schema warmup failed: ${err instanceof Error ? err.message : String(err)}`),
    );

    initGlobalEventSystem();
  } else {
    // HTTP mode: multi-session server
    ...
    // Eagerly load all lazy tool schemas before accepting requests.
