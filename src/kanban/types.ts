...
export interface KanbanTask {
  ...
  parentTaskId?: string;
  /** Child subtask IDs (denormalized for quick access) */
  subtaskIds?: string[];
}
...
// ============================================================================
// BOARD WORKFLOW TYPES (Auto Board Switching)
// ============================================================================

/**
 * Board Workflow - defines automatic flow between boards
 * When all tasks on a board are done, automatically switch to next board
 */
export interface BoardWorkflow {
  ...
  /** Session ID */
  sessionId: string;
  /** Ordered list of board IDs to process */
  boardSequence: string[];
  /** Current board index in sequence */
  currentIndex: number;
  /** Workflow status */
  status: 'active' | 'paused' | 'completed';
  /** Auto-create navigation task when board completes */
  autoCreateNavigationTask: boolean;
  /** Created by agent */
  createdBy: string;
  /** Creation timestamp */
  createdAt: number;
...
export const KANBAN_KEYS = {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // Workflow Keys (Auto Board Switching)
  // ─────────────────────────────────────────────────────────────────────────────

  /** HASH: workflow data - mcp:kanban:workflow:{workflowId} */
  workflow: (workflowId: string) => `mcp:kanban:workflow:${workflowId}`,
  /** SET: workflows in session - mcp:kanban:workflows:session:{sessionId} */
  workflowsBySession: (sessionId: string) => `mcp:kanban:workflows:session:${sessionId}`,
  /** STRING: active workflow for board - mcp:kanban:workflow:board:{boardId} */
  workflowByBoard: (boardId: string) => `mcp:kanban:workflow:board:${boardId}`,

  // ─────────────────────────────────────────────────────────────────────────────
  // Subtask Keys
  // ─────────────────────────────────────────────────────────────────────────────

  /** SET: subtask IDs for parent task - mcp:kanban:task:{taskId}:subtasks */
  subtasks: (taskId: string) => `mcp:kanban:task:${taskId}:subtasks`,

  // ─────────────────────────────────────────────────────────────────────────────
  // Directive Keys (Obligatory AI Actions)
  // ─────────────────────────────────────────────────────────────────────────────

  /** LIST: pending directives for session - mcp:kanban:directives:{sessionId} */
  directives: (sessionId: string) => `mcp:kanban:directives:${sessionId}`,
  /** HASH: directive data - mcp:kanban:directive:{directiveId} */
  directive: (directiveId: string) => `mcp:kanban:directive:${directiveId}`,
};
...
// ============================================================================
// DIRECTIVE TYPES (Obligatory AI Actions)
// ============================================================================

/**
 * Directive priority levels
 * Determines how urgently the AI should act on the directive
 */
export type DirectivePriority = 'critical' | 'high' | 'normal';

/**
 * Directive - an obligatory action the AI must perform
 *
 * @description Directives are injected into tool responses to ensure
 *              the AI cannot miss important actions. They appear as
 *              prefixes in the response with special formatting.
 */
export interface Directive {
  /** Unique directive ID */
  id: string;
  /** Session ID this directive belongs to */
  sessionId: string;
  /** Priority level */
  priority: DirectivePriority;
  /** Short title/action name */
  title: string;
  /** Detailed instruction for the AI */
  instruction: string;
  /** Source of directive (workflow, system, user, etc.) */
  source: string;
  /** Optional context data (e.g., target boardId, taskId) */
  context?: Record<string, unknown>;
  /** Creation timestamp */
  createdAt: number;
  /** Expiration timestamp (auto-cleanup) */
  expiresAt: number;
  /** Whether this directive has been delivered */
  delivered: boolean;
  /** When the directive was delivered */
  deliveredAt?: number;
  /** Tool that delivered this directive */
  deliveredBy?: string;
}

/** Directive creation input */
...
/** Directive prefixes by priority */
export const DIRECTIVE_PREFIXES: Record<DirectivePriority, string> = {
  critical: '🚨 [CRITICAL ACTION REQUIRED - DO NOT SKIP]',
  high: '⚠️ [OBLIGATORY ACTION - DO NOT SKIP]',
