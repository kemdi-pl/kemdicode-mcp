...
export interface AgenticLoopConfig {
  ...
  /** Board ID for kanban task creation (injected into prompt so model knows where to record findings) */
  boardId?: string;
}
 ...
 * Matches: [TOOL_CALL]...[/TOOL_CALL], ```json {"tool":"name",...}```,
 * and XML-style <tool_call>...</tool_call> or <prefix:tool_call>...</prefix:tool_call> patterns.
 * Input is already truncated to MAX_PARSE_LENGTH before these are applied.
 */
...
/**
 * Regex to detect XML-based tool call blocks in AI responses.
 * Matches any tag ending in "tool_call" with optional namespace prefix.
 * Captures the full inner content for parsing by fast-xml-parser.
 */
const XML_TOOL_CALL_BLOCK_REGEX = /<((?:\w+:)?tool_call)>([\s\S]*?)<\/\1>/g;
 ...
 * Counts braces to handle nested objects/arrays correctly.
 */
function extractBalancedJSON(str: string, start: number): string | null {
  ...
  let inString = false;
  let escape = false;
  for (let i = start; i < str.length && i < start + 50000; i++) {
    const ch = str[i];
...
// ---------------------------------------------------------------------------
// Generic XML tool call parser (model-agnostic)
// ---------------------------------------------------------------------------

/** Lazily initialized XML parser instance */
...
/**
 * Parse XML-based tool calls generically using fast-xml-parser.
 *
 * Handles ANY XML structure that contains tool call information:
 * - `<tool_call>{"tool":"x","args":{...}}</tool_call>` — JSON body
 * - `<tool_call><invoke name="x"><parameter name="k">v</parameter></invoke></tool_call>` — XML params
 * - `<tool_call><invoke name="x", "args": {json}>...</invoke></tool_call>` — hybrid (MiniMax etc.)
 * - Any namespace prefix: `<minimax:tool_call>`, `<llm:tool_call>`, etc.
 *
 * @param xmlContent - The inner content of a `<*tool_call>` block
 * @param fullBlock - The full XML block including outer tags (for fallback)
 * @returns Array of parsed tool calls
 */
function parseXmlToolCalls(xmlContent: string, fullBlock: string): ToolCall[] {
  const calls: ToolCall[] = [];

  // --- Strategy 1: Body is plain JSON (no XML children) ---
  ...
  // --- Strategy 2: Hybrid format — <invoke name="tool", "args": {json}> ---
  // Detect args embedded in the opening <invoke> tag (non-standard but common)
  const invokeArgsRegex = /<invoke\s+name="([\w-]+)"[,\s]*"args"\s*:\s*/g;
  let invokeMatch: RegExpExecArray | null;
  while ((invokeMatch = invokeArgsRegex.exec(fullBlock)) !== null) {
    const toolName = invokeMatch[1];
    const jsonStart = invokeMatch.index + invokeMatch[0].length;
    const jsonStr = extractBalancedJSON(fullBlock, jsonStart);
    if (jsonStr) {
      try {
        const args = JSON.parse(jsonStr) as Record<string, unknown>;
        calls.push({ tool: toolName, args: sanitizeParsedJSON(args) as Record<string, unknown>, id: uuidv4() });
      } catch {
        Logger.warn(`agentic-loop: XML hybrid parse failed for "${toolName}": ${jsonStr.slice(0, 80)}`);
      }
    }
  }
  if (calls.length > 0) return calls;

  // --- Strategy 3: Standard XML with <invoke> and <parameter> children ---
  try {
    const parser = getXmlParser();
    // Wrap in root to handle fragments
    const xml = `<root>${xmlContent}</root>`;
    const doc = parser.parse(xml);
    const root = doc.root;
    if (!root) return calls;

    // Find <invoke> elements (may be nested under various paths)
    const invokes = findInvokes(root);
    for (const inv of invokes) {
      const toolName = inv['@_name'];
      if (!toolName || typeof toolName !== 'string') continue;

      const args: Record<string, unknown> = {};

      // Extract <parameter> children
      const params = inv.parameter;
      if (params) {
        const paramList = Array.isArray(params) ? params : [params];
        for (const p of paramList) {
          const key = p['@_name'];
          if (!key) continue;
          let value: unknown = typeof p === 'object' && '#text' in p ? p['#text'] : p;
          // Try JSON coercion for structured values
          if (typeof value === 'string') {
            const vt = value.trim();
            if (vt.startsWith('{') || vt.startsWith('[') || vt === 'true' || vt === 'false' || !isNaN(Number(vt))) {
              try { value = JSON.parse(vt); } catch { /* keep as string */ }
            }
          }
          args[key] = value;
        }
    ...
    Logger.debug(`agentic-loop: XML parse failed, skipping: ${err instanceof Error ? err.message : String(err)}`);
  }
...
/**
 * Recursively find all <invoke> elements in a parsed XML tree.
 */
function findInvokes(node: unknown): Record<string, unknown>[] {
  const results: Record<string, unknown>[] = [];
  if (!node || typeof node !== 'object') return results;

  if (Array.isArray(node)) {
    for (const item of node) {
      results.push(...findInvokes(item));
    }
    return results;
  }
  ...
  // Check if this node itself IS an invoke (has @_name)
  if ('@_name' in obj) {
    results.push(obj);
    return results;
  }

  // Look for 'invoke' key (with or without namespace prefix)
  for (const key of Object.keys(obj)) {
    if (key === 'invoke' || key.endsWith(':invoke')) {
      const val = obj[key];
...
export function parseToolCalls(response: string): ToolCall[] {
  ...
  if (calls.length === 0) {
    // Generic XML tool call parser — handles all XML formats (JSON body, <parameter> tags,
    // hybrid args-in-attribute) via fast-xml-parser. Model-agnostic: works with any namespace.
    XML_TOOL_CALL_BLOCK_REGEX.lastIndex = 0;
    while ((match = XML_TOOL_CALL_BLOCK_REGEX.exec(response)) !== null) {
      const xmlContent = match[2];
      const fullBlock = match[0];
      const parsed = parseXmlToolCalls(xmlContent, fullBlock);
      for (const call of parsed) calls.push(call);
    }
  }

  if (calls.length === 0) {
    // Standalone <invoke> without wrapping <tool_call> — some models emit just <invoke>
    const standaloneInvokeRegex = /<invoke\s+name="([\w-]+)"[^>]*(?:>[\s\S]*?<\/invoke>|[,\s]*"args"\s*:\s*\{)/g;
    if (standaloneInvokeRegex.test(response)) {
      standaloneInvokeRegex.lastIndex = 0;
      // Wrap in synthetic tool_call and parse
      const syntheticBlock = `<tool_call>${response}</tool_call>`;
      const parsed = parseXmlToolCalls(response, syntheticBlock);
      for (const call of parsed) calls.push(call);
    }
  }
...
IMPORTANT: Always use JSON format as shown above. Do NOT use XML, do NOT use <tool_call>, <invoke>, or <parameter> tags.
...
\`\`\`json
{"action": "create", "tasks": [{"title": "[FINDING] Short description", "description": "Detailed explanation with file:line references", "priority": "high"${config.boardId ? `, "boardId": "${config.boardId}"` : ''}}]}
\`\`\`
IMPORTANT: priority MUST be one of: "critical", "high", "normal", "low". No other values are accepted.
${config.boardId ? `IMPORTANT: Always include "boardId": "${config.boardId}" in every task you create.` : ''}
...
3. RECORD: For EACH distinct finding, create a separate kanban task AND share via collaboration tools. Create tasks IMMEDIATELY when you find something — do NOT wait until the end.
4. SYNTHESIZE: Only after using most of your iteration budget AND recording all findings, provide your final answer.

CRITICAL: Create kanban tasks during ANALYZE/RECORD phases, NOT at the end. If you wait until the end, your response may be truncated and tasks will be lost.

## Tool Call Format
To call a tool, emit EXACTLY this format (plain JSON in a tool-call code block):
\`\`\`tool-call
{"tool": "tool-name", "args": {"key": "value"}}
\`\`\`
Do NOT use XML tags, do NOT use <tool_call> or <invoke> or <parameter> tags. Use ONLY the JSON format above.
