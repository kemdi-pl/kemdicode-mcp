...
async function incrementRateLimit(agentId: string): Promise<number> {
  ...
  try {
    ...
    // Direct in-memory fallback — do NOT recurse into incrementRateLimit
    const now = Date.now();
    const memKey = `memory:ratelimit:${agentId}`;
    const memEntry = memoryRateLimits.get(memKey);
    if (!memEntry || now >= memEntry.resetAt) {
      memoryRateLimits.set(memKey, { count: 1, resetAt: now + RATE_LIMIT_WINDOW });
      return 1;
    }
    const newCount = memEntry.count + 1;
    memoryRateLimits.set(memKey, { ...memEntry, count: newCount });
    return newCount;
  }
}
 ...
 */
async function _getRateLimitEntry(agentId: string): Promise<RateLimitEntry> {
  const client = await tryGetRedis();
  if (!client) {
    // In-memory fallback
    const key = `memory:ratelimit:${agentId}`;
...
export async function checkSafety(
  ...
  // Atomic rate limit: increment-then-check to prevent TOCTOU race
  // This ensures concurrent calls both see the incremented count
  let currentCount: number;
  try {
    currentCount = await incrementRateLimit(request.agentId);
  } catch {
    // If rate limiting infrastructure fails, allow through with count 0
    currentCount = 0;
  }

  const resetIn = RATE_LIMIT_WINDOW;

  if (currentCount > policy.maxInvocationsPerMinute) {
    return {
      allowed: false,
...
export async function invokeTool(
  ...
  try {
    // Rate limit already incremented atomically in checkSafety()
...
> {
  ...
  return entries
    .map((e) => safeJsonParseNoProto<{
      invocationId: string; toolName: string; success: boolean; error?: string;
      duration: number; depth: number; timestamp: number;
    }>(e))
