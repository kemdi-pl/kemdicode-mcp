...
export class AgentMonitor {
  ...
  async getConversationSnapshot(
    ...
    // Use local map to avoid race conditions with concurrent calls
    const dedupeMap = new Map<string, (typeof alertSets)[0][0]>();
    for (const alert of alertSets.flat()) {
      dedupeMap.set(alert.id, alert);
    }
    const pendingAlerts = [...dedupeMap.values()];
