...
You MUST respond with ONLY a valid JSON object. No markdown, no XML, no explanation — raw JSON only:
{
...
export class AgentIterationLoop {
  ...
  async execute(prompt: string, systemPrompt?: string): Promise<AgentIterationResult> {
    ...
    // eslint-disable-next-line no-useless-assignment -- assigned in Pass 1 below
    let bestContent = '';
    let bestQuality = 0;
    let passNumber = 0;
