...
import { getAuditScheduler, resetAuditScheduler } from './audit-scheduler.js';
...
export async function initClusterBusSystem(): Promise<boolean> {
  ...
  try {
        ...
        if (payload.orchestrate) {
          ...
          (async () => {
            ...
            const result = await executeAgenticLoop({
              ...
              totalClusters: orchConfig.totalClusters,
              clusterIndex: orchConfig.clusterIndex,
              clusterFocus: orchConfig.clusterFocus,
              boardId: orchConfig.boardId,
    ...
    // 12. Initialize audit scheduler (loads persisted schedules from Redis)
    const auditScheduler = getAuditScheduler();
    auditScheduler.init().catch((err) => {
      Logger.warn(`[ClusterBus] Audit scheduler init failed: ${err instanceof Error ? err.message : String(err)}`);
    });
