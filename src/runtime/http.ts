...
/** Maximum HTTP request body size (5MB) — prevents DoS via oversized payloads */
...
async function wrapNodeRequest(req: import('node:http').IncomingMessage): Promise<UnifiedRequest> {
  // Read body for POST requests (with size limit)
    ...
    const server = Bun.serve({
      ...
      fetch: async (request: Request) => {
        ...
        if (request.method === 'POST' || request.method === 'PUT' || request.method === 'PATCH') {
          ...
          if (unifiedReq.body.length > MAX_BODY_SIZE) {
            return new Response(JSON.stringify({ error: 'Payload too large' }), {
              status: 413,
              headers: { 'Content-Type': 'application/json' },
            });
