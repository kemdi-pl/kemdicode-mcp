...
export interface Decision {
  ...
  sessionId: string;
  agentId: string;
  /** Orchestration loop that created this decision (for traceability) */
...
export interface MentalModel {
  id: string;
  sessionId: string;
  /** Agent that created/last updated this model */
  agentId?: string;
  orchestrationId?: string;
...
export interface DriftAlert {
  ...
  driftScore: number;
  /** Human-readable explanation of why drift was detected */
  reason?: string;
}
...
export interface HandoffReport {
  ...
  confidence: number;
  /** CTC boundary encoding: information density per field relative to intent */
  boundaryEncoding?: Record<string, number>;
}
...
export const COGNITION_KEYS = {
  ...
  // CTC (Closed Timelike Curve) extensions
  causalCore: (sessionId: string) => `mcp:cognition:causal-core:${sessionId}`,
  handoffBoundary: (id: string) => `mcp:cognition:handoff:boundary:${id}`,
...
/** Result of causal core analysis */
export interface CausalCoreResult {
  /** Set of "type:id" strings for items in the causal core */
  coreItems: Set<string>;
  /** Total items analyzed */
  totalItems: number;
  /** Session ID this applies to */
  sessionId: string;
  /** Timestamp of computation */
  computedAt: number;
}

/** Cross-link consistency violation (dangling pointer) */
export interface ConsistencyViolation {
  /** The link with a missing target */
  link: CognitionLink;
  /** Whether the target key is missing from Redis */
  missingTarget: boolean;
  /** Suggested replacement "type:id", if found via A* repair */
  repairSuggestion?: string;
}

/** Result of thinking chain compaction via Nowikov fixed-point */
export interface ChainCompactionResult {
  chainId: string;
  originalThoughtCount: number;
  keptCount: number;
  prunedCount: number;
  fixedPointIndex: number;
  /** Fraction of thoughts above prune threshold (0-1) */
