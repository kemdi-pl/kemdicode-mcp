...
const schema = z.object({
  ...
  limit: z.number().min(1).max(100).default(50).describe('Max agents to return'),
});

export const agentListTool: UnifiedTool = {
  name: 'agent-list',
  description: 'List active agents with IDs, roles, status, and tasks. Use when: checking which agents are available or monitoring agent activity.',
  zodSchema: schema,
  ...
  metadata: {
    ...
    relatedTools: ['agent-register', 'monitor'],
  },

  execute: async (args) => {
    const { sessionId, includeIdle, offset, limit } = args as z.infer<typeof schema>;
    ...
    if (paginated.length === 0) {
      ...
      return JSON.stringify({ success: true, total, offset, limit, hasMore: false, count: 0, agents: [] });
    }

    if (isSilent()) {
      return JSON.stringify(paginated.map((a) => ({ id: a.id, role: a.role, status: a.status })));
    }
    ...
    lines.push('╚══════════════════════════════════════════════════════════════════╝');
    lines.push(`\nTotal: ${total} agent(s) | Showing: ${paginated.length} (offset: ${offset}) | Has more: ${hasMore}`);
