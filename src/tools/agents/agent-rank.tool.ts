...
export const agentRankTool: UnifiedTool<typeof schema> = {
  name: 'agent-rank',
  description: 'View and manage agent performance rankings with bronze-to-diamond tier system',
  zodSchema: schema,
  metadata: {
    category: 'agents',
  ...
  execute: async (args) => {
    ...
    switch (action) {
      case 'get-rank': {
        if (!agentId) return 'Error: agentId required for get-rank action.';
        const score = await store.getScore(agentId);
        if (!score) return `No ranking data found for agent "${agentId}".`;
        return `# Agent Ranking\n\n${formatScore(score)}`;
      }

      case 'leaderboard': {
        const board = await store.getLeaderboard(limit);
        if (board.length === 0) return 'No agents ranked yet. Rankings update automatically as agents execute tools.';
      ...
      case 'promote': {
        if (!agentId) return 'Error: agentId required for promote action.';
        const promoted = await store.promote(agentId);
        if (!promoted) return `Agent "${agentId}" not found.`;
        return `Agent "${agentId}" promoted to ${promoted.rank}.\n\n${formatScore(promoted)}`;
      }

      case 'demote': {
        if (!agentId) return 'Error: agentId required for demote action.';
        const demoted = await store.demote(agentId);
        if (!demoted) return `Agent "${agentId}" not found.`;
        return `Agent "${agentId}" demoted to ${demoted.rank}.\n\n${formatScore(demoted)}`;
      }

      case 'history': {
        if (!agentId) return 'Error: agentId required for history action.';
        const score = await store.getScore(agentId);
        if (!score) return `No ranking data found for agent "${agentId}".`;

        if (score.history.length === 0) return `No rank changes recorded for agent "${agentId}".`;
