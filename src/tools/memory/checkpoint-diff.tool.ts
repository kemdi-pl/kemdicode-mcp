...
const schema = z.object({
  name: z.string().min(1).describe('Checkpoint name to compare against (same name used in checkpoint-save)'),
  files: z
    .array(z.string())
    .optional()
...
export const checkpointDiffTool: UnifiedTool = {
  ...
  description:
    'Compare current file state with a saved checkpoint to see what changed. ' +
    'Use when: reviewing changes since a checkpoint, or verifying expected modifications.',
  zodSchema: schema,
  metadata: {
    category: 'memory',
    tags: ['checkpoint', 'diff', 'compare'],
    examples: [
      { args: { name: 'before-refactor' }, description: 'Compare all files against a saved checkpoint' },
      { args: { name: 'v1-release', files: ['src/index.ts', 'src/server.ts'] }, description: 'Diff specific files against checkpoint' },
    ],
    relatedTools: ['checkpoint-save', 'checkpoint-restore'],
  },

  execute: async (args): Promise<string> => {
    const { name: checkpointId, files: requestedFiles } = args as CheckpointDiffArgs;
