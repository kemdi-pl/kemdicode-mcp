...
export const writeMemoryTool: UnifiedTool = {
  name: 'write-memory',
  description:
    'Store 1-20 named memories for current project. ' +
    'USE PROACTIVELY: save key findings, architectural decisions, and session state for future sessions. ' +
    'Memory persists across sessions — use it to build cumulative project knowledge.',
  zodSchema: schema,
  metadata: {
    category: 'memory',
  ...
  execute: async (args): Promise<string> => {
    ...
    return executeWithGuard('write-memory', 'memory-operations', async () => {
