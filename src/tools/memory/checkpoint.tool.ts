...
async function readFileOrNull(filePath: string): Promise<string | null> {
  try {
    // Validate path against traversal attacks before reading
    ...
    if (!validation.ok) {
      Logger.warn(`[Checkpoint] Path validation failed for "${filePath}": ${validation.error}`);
      return null;
    }

    await access(validation.path);
    return await readFile(validation.path, 'utf-8');
  } catch {
    return null;
  }
}
