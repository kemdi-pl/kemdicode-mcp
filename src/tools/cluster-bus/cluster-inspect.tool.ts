...
export const clusterInspectTool: UnifiedTool<typeof schema> = {
  ...
  description:
    'Deep inspection: signal history, health events, bridge stats, subscription diagnostics, bloom filter, CI fan-in aggregations',
  zodSchema: schema,
  metadata: {
    ...
    examples: [
      ...
      { args: { view: 'aggregations' }, description: 'CI fan-in aggregation results and pending pipelines' },
    ...
    switch (args.view) {
      ...
      case 'bridge-stats': {
        ...
        return [
          ...
          '',
          `**Total bridged:** ${stats.eventsToCluster + stats.clusterToEvents}`,
        ].join('\n');
      }
      ...
      case 'aggregations': {
        ...
        const pending = getPendingAggregations();

        if (all.length === 0) {
          return 'No aggregations found. Send a `ci:multicast` signal to start one.';
        }

        const lines = [
          '# CI Fan-In Aggregations',
          '',
          `**Total:** ${all.length} (${pending.length} pending)`,
          '',
          '| Pipeline | Stage | State | Received | Success | Failed | Duration |',
          '|----------|-------|-------|----------|---------|--------|----------|',
        ];

        for (const a of all) {
          lines.push(
            `| ${a.pipelineId} | ${a.stage} | **${a.state}** | ${a.receivedCount}/${a.totalExpected} | ${a.successCount} | ${a.failureCount} | ${a.durationMs > 0 ? `${a.durationMs}ms` : 'ongoing'} |`,
          );
        }

        // Show details of each aggregation's results
        for (const a of all) {
          if (a.results.length > 0) {
            lines.push('', `### ${a.pipelineId} — Results`);
            for (const r of a.results) {
              const status = r.success ? 'pass' : 'FAIL';
              const detail = r.success
                ? (r.output ? JSON.stringify(r.output).slice(0, 100) : 'ok')
                : (r.error || 'unknown error');
              lines.push(`- **${r.targetCluster}**: ${status} (${r.durationMs}ms) — ${detail}`);
            }
          }
        }
      ...
      case 'full-report': {
        ...
        if (bridge) {
          ...
          const total = bStats.eventsToCluster + bStats.clusterToEvents;
          lines.push(
            '',
            '## Bridges',
