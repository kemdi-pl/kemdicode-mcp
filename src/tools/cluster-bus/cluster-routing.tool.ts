...
const schema = z.object({
  action: z
    ...
    .describe('Routing or provider pool action'),
  ...
  predicates: z
    .array(
      z.object({
        ...
        mode: z.enum(['exact', 'wildcard', 'regex', 'prefix']).default('exact').describe('Match mode'),
      }),
    )
    .optional()
    .describe('Tag predicates for routing'),
  logic: z.enum(['and', 'or']).optional().describe('Predicate combination logic'),
  ...
  preset: z
    ...
    .optional()
    .describe('Preset routing rule builder'),
