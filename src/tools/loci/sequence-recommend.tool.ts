...
export const sequenceRecommendTool: UnifiedTool<typeof schema> = {
  ...
  description: 'Get tool recommendations based on learned execution patterns. USE PROACTIVELY: get tool sequence recommendations based on learned usage patterns.',
  zodSchema: schema,
  ...
  execute: async (args) => {
    const { agentId, sessionId: explicitSessionId, showPatterns, showTransitions } = args;
