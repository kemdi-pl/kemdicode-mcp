...
const schema = z.object({
  ...
  level: z.enum(['L0', 'L1', 'L2', 'L3']).default('L0').describe('Compaction level (action=timeline): L0 (raw events, no compaction), L1 (minor compaction), L2 (moderate compaction), L3 (heavy compaction, summaries only)'),
  ...
  insightType: z.enum(['sequences', 'file-relations', 'time-patterns', 'agent-effectiveness', 'all']).default('all')
    .describe('Type of ambient insight (action=ambient-insights): sequences (common tool sequences), file-relations (frequently co-edited files), time-patterns (activity patterns), agent-effectiveness (agent performance), all (everything)'),
...
export const lociRecallTool: UnifiedTool<typeof schema> = {
  name: 'loci-recall',
  description:
    'Recall memories from memory palace, view timeline, resurrect context after compaction, or link sessions. ' +
    'USE PROACTIVELY: "resurrect" after context compaction to recover key knowledge. ' +
    '"walk" at session start to see what knowledge exists. "ambient-insights" to discover patterns you missed.',
  zodSchema: schema,
  ...
  execute: async (args) => {
    const { action, lociId, lociName, sessionId: explicitSessionId, walkAll } = args;
