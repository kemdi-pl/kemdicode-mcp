...
const schema = z.object({
  ...
  format: z.enum(['detailed', 'summary', 'timeline']).default('summary').describe('Output format: detailed (full outputs per entry), summary (compact overview), timeline (chronological with timestamps)'),
...
export const sharedThoughtsTool: UnifiedTool = {
  ...
  description:
    'Read collective knowledge from all agents. Shows analyzed data for multi-agent coordination. USE PROACTIVELY: read shared context at session start to understand what other agents have learned.',
  zodSchema: schema,
