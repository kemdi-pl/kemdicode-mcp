...
const schema = z.object({
  action: z
    ...
    .describe('Action: stats (feedback statistics), insights (learning insights), check (verify suggestion adoption), iteration (current iteration status), start (begin new iteration), complete (finish iteration), suggest-fix (get fix for error), file-history (suggestions for a file), rate (rate a suggestion)'),
  ...
  rating: z.enum(['helpful', 'not_helpful', 'partial']).optional().describe('Rating for a suggestion (action=rate): helpful (worked well), not_helpful (did not help), partial (partially useful)'),
...
export const feedbackTool: UnifiedTool = {
  name: 'feedback',
  description:
    'Access feedback loop for continuous improvement. ' +
    'USE PROACTIVELY: "suggest-fix" when encountering errors, "start"/"complete" to track work iterations, ' +
    '"insights" to learn from past sessions, "rate" suggestions to improve future recommendations.',
  zodSchema: schema,
  ...
  metadata: {
    ...
    relatedTools: ['error-pattern', 'self-critique'],
  },
