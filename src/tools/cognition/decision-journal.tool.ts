    ...
    return executeCognitionTool('decision-journal', getDecisionStore, async (store) => {
      switch (input.action) {
        ...
        case 'list': {
          ...
          if (decisions.length === 0) {
            return [
              '## Decision Journal',
              '',
              `No decisions found for session \`${input.sessionId}\`.`,
            ].join('\n');
          }
          ...
          const lines: string[] = [
            '## Decision Journal',
            '',
            `**Session:** \`${input.sessionId}\` | **Showing:** ${decisions.length} decision(s)`,
            '',
        ...
        case 'search': {
          ...
          const results = await store.search(input.query, input.sessionId);
          ...
          if (limited.length === 0) {
            return [
              '## Decision Search',
              '',
              `No decisions found matching **"${input.query}"**${input.sessionId ? ` in session \`${input.sessionId}\`` : ''}.`,
            ].join('\n');
          }
