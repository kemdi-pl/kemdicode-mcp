...
const schema = z.object({
  action: z
    ...
    .describe('Action: create (new model), get (by name/ID), list (browse models), add-component (add to model), add-relationship (link components), remove-component, check-staleness (detect outdated models), refresh (update from code), impact-analysis (what does changing X affect), dependency-chain (trace dependencies), invariant-check (verify invariants hold)'),
  ...
  // create: inline components and relationships (avoids multiple calls)
  components: z
    .array(
      z.object({
        name: z.string().max(200).describe('Component name'),
        role: z.string().max(1000).default('').describe('Component role/responsibility'),
        files: z.array(z.string().max(500)).max(50).default([]).describe('Source files'),
        dependencies: z.array(z.string().max(200)).max(30).default([]).describe('Dependency names'),
        invariants: z.array(z.string().max(500)).max(20).default([]).describe('Key invariants'),
      })
    )
    .max(50)
    .optional()
    .describe('Inline components to add on create (saves multiple add-component calls)'),
  relationships: z
    .array(
      z.object({
        from: z.string().max(200).describe('Source component'),
        to: z.string().max(200).describe('Target component'),
        type: z.enum(['calls', 'depends-on', 'produces', 'consumes', 'extends', 'contains']).describe('Relationship type: calls (invokes), depends-on (requires), produces (outputs data), consumes (reads data), extends (inherits), contains (parent-child)'),
        protocol: z.string().max(200).optional().describe('Communication protocol'),
        description: z.string().max(500).optional().describe('Relationship description'),
      })
    )
    .max(100)
    .optional()
    .describe('Inline relationships to add on create (saves multiple add-relationship calls)'),
...
export const mentalModelTool: UnifiedTool<typeof schema> = {
  ...
  description:
    ...
    'USE PROACTIVELY: "get" existing models before modifying unfamiliar code. "check-staleness" after major refactors. ' +
    '"create" models when exploring a new codebase to preserve understanding for future sessions.',
  zodSchema: schema,
  ...
  metadata: {
    ...
    relatedTools: ['thinking-chain', 'memory', 'graph-query', 'decision-journal'],
  },
    ...
    return executeCognitionTool('mental-model', getMentalModelStore, async (store) => {
      switch (input.action) {
        ...
        case 'create': {
          ...
          // Add inline components if provided
          let addedComponents = 0;
          let addedRelationships = 0;
          if (input.components && input.components.length > 0) {
            for (const comp of input.components) {
              const ok = await store.addComponent(model.id, {
                name: comp.name,
                role: comp.role || '',
                files: comp.files || [],
              ...
              if (ok) addedComponents++;
            }
          }

          // Add inline relationships if provided
          if (input.relationships && input.relationships.length > 0) {
            for (const rel of input.relationships) {
              const ok = await store.addRelationship(model.id, {
                from: rel.from,
                to: rel.to,
                type: rel.type,
              ...
              if (ok) addedRelationships++;
            }
          }

          Logger.debug(`mental-model: created model ${model.id} "${model.name}" with ${addedComponents} components, ${addedRelationships} relationships`);
          ...
          return JSON.stringify({
            ...
            message: addedComponents > 0 || addedRelationships > 0
              ? `Model "${model.name}" created with ${addedComponents} component(s) and ${addedRelationships} relationship(s).`
              : `Model "${model.name}" created. Add components with action=add-component.`,
          });
        }
