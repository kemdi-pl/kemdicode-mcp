 ...
 * sub-goal, and task. Drift detection uses TF-IDF weighted term
 * similarity combined with bigram sequence similarity to compare
 * current agent actions against the active intent.
 *
...
const schema = z.object({
  action: z
    ...
    .describe('Action: set (create intent), get (by ID), check-drift (compare action vs intent), hierarchy (show chain), list (browse), complete/abandon (update status)'),
  ...
  level: z
    ...
    .optional()
    .describe('Intent level: mission (long-term), goal (session-level), sub-goal, task (immediate). Required for action=set.'),
  description: z
    .string()
    .min(5)
    .max(1000)
    .optional()
    .describe('What to achieve - used for drift detection via TF-IDF + bigram similarity scoring. Required for action=set (5-1000 chars).'),
  parentIntentId: z
    .string()
    .min(1)
    .optional()
    .describe('Parent intent ID for hierarchy building (action=set)'),
  tags: z
    .array(z.string().min(1).max(30))
    .max(10)
    .optional()
    .describe('Tags for categorization (action=set, max 10)'),
  ...
  intentId: z
    ...
    .min(1)
    .optional()
    .describe('Intent ID (required for action=get, hierarchy, complete, abandon)'),
  ...
  currentAction: z
    ...
    .max(500)
    .optional()
    .describe('What agent is currently doing - compared against active intent using TF-IDF weighted term + bigram sequence similarity (score 0=aligned, 1=divergent, >0.7 auto-marks drifted). Required for action=check-drift.'),
  threshold: z
    .number()
    .min(0)
    .max(1)
    .optional()
    .describe('Drift score threshold above which an alert fires (default 0.5). Lower values are more sensitive.'),
...
export const intentTrackerTool: UnifiedTool<typeof schema> = {
  ...
  description:
    ...
    'USE PROACTIVELY: "set" intent at task start, "check-drift" periodically during long tasks, ' +
    '"hierarchy" to review goal tree when context gets complex. Drift score >0.7 means you are off track.',
  zodSchema: schema,
      ...
      switch (input.action) {
        ...
        case 'get': {
          const intent = input.intentId
            ? await store.get(input.intentId)
            : await store.getActiveIntent(sessionId);
        ...
        case 'check-drift': {
          ...
          const activeIntent = await store.getActiveIntent(sessionId);
          if (!activeIntent) {
            return JSON.stringify({
              success: true,
          ...
          const driftAlert = await store.checkDrift(sessionId, input.currentAction, input.threshold);
          ...
          if (driftAlert) {
            ...
            if (driftAlert.reason) {
              lines.push('');
              lines.push(`Analysis:          ${driftAlert.reason}`);
            }

            lines.push('');
            lines.push('RECOMMENDATION: Realign your current work with the original intent.');
            lines.push(`Intent ID:         ${activeIntent.id}`);
            lines.push(`Intent level:      ${activeIntent.level}`);
        ...
        case 'hierarchy': {
          if (!input.intentId) {
            // Try active intent
            const active = await store.getActiveIntent(sessionId);
            if (!active) {
              return JSON.stringify({
                success: false,
        ...
        // ─────────────────────── LIST ─────────────────────────────
        case 'list': {
          const intents = await store.listBySession(sessionId, input.limit);
