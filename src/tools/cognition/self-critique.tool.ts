...
const schema = z.object({
  action: z.enum(['reflect', 'get', 'list', 'lessons', 'trend', 'check-application']).describe('Action: reflect (write post-session critique), get (retrieve by ID), list (browse critiques), lessons (extracted lessons across sessions), trend (efficiency over time), check-application (verify if lessons are being applied)'),
  ...
  // reflect
  scope: z.enum(['session', 'task', 'decision']).optional().describe('What is being critiqued (action=reflect): session (entire session), task (specific task), decision (specific decision)'),
...
export const selfCritiqueTool: UnifiedTool<typeof schema> = {
  name: 'self-critique',
  description:
    'Post-session reflection — review what went well, what went poorly, and extract lessons. ' +
    'USE PROACTIVELY: run at session end, after completing complex tasks, or after encountering repeated errors. ' +
    'Check "lessons" before starting new tasks to avoid repeating mistakes.',
  zodSchema: schema,
  ...
  metadata: {
    ...
    relatedTools: ['thinking-chain', 'decision-journal', 'memory', 'feedback'],
  },
    ...
    return executeCognitionTool('self-critique', getSelfCritiqueStore, async (store) => {
      switch (input.action) {
        ...
        case 'reflect': {
          ...
          Logger.debug(`self-critique: recorded ${critique.id} for session ${sessionId}`);
        ...
        case 'list': {
          ...
          if (input.scope) {
            ...
            critiques = await store.listBySession(sessionId, limit);
          }
        ...
        // ───────────────────────────────────────────────────────── trend
        case 'trend': {
          const trend = await store.getEfficiencyTrend(sessionId);
          ...
          if (trend.length === 0) {
            return JSON.stringify({
              ...
              count: 0,
              trend: [],
              message: `No efficiency data for session ${sessionId}.`,
            });
          }
          ...
          lines.push(`\u2502 ${`Session: ${sessionId}`.substring(0, w - 1).padEnd(w - 1)}\u2502`);
          lines.push(`\u251C${hr}\u2524`);
        ...
        case 'check-application': {
          ...
          const recentDecisions = await decisionStore.listBySession(sessionId, 20);
