...
export const batchTool: UnifiedTool<typeof schema> = {
  ...
  metadata: {
    ...
    examples: [
      { args: { operations: [{ tool: 'find-definition', args: { symbol: 'Logger' } }, { tool: 'find-definition', args: { symbol: 'Redis' } }], parallel: true }, description: 'Find two symbol definitions in parallel' },
      { args: { operations: [{ tool: 'memory', args: { action: 'read', names: ['config'] } }, { tool: 'memory', args: { action: 'list', limit: 5 } }], parallel: true }, description: 'Read memory and list memories simultaneously' },
    ],
