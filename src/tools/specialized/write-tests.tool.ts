...
const schema = z.object({
  ...
  type: z.enum(['unit', 'feature', 'both']).default('unit').describe('Test type: unit (isolated method tests with mocks), feature (HTTP flow tests with auth), both (generate both types)'),
  coverage: z.enum(['happy-path', 'edge-cases', 'full']).default('happy-path').describe('Coverage level: happy-path (main success scenarios), edge-cases (nulls, boundaries, invalid input), full (happy + edge + error handling + auth + DB + events)'),
});

export const writeTestsTool: UnifiedTool = {
  name: 'write-tests',
  description:
    'Generate PHPUnit tests for PHP/Laravel code (PHP-only, not for TypeScript/JavaScript/Python). ' +
    'Outputs test file content with PHPUnit assertions, mocks, and Laravel helpers. ' +
    'USE PROACTIVELY: generate tests after writing new features or fixing bugs to prevent regressions.',
  zodSchema: schema,
  ...
  metadata: {
    ...
    longRunning: true,
    aiRequired: { fallbackTools: ['find-definition', 'find-references'] },
    aiRouting: 'hybrid',
    examples: [
    ...
    relatedTools: ['find-definition', 'find-references', 'semantic-search'],
  },
