...
const AUTO_SAVE_SKIP_TOOLS: ReadonlySet<string> = new Set([
  'ping', 'help', 'read-memory', 'write-memory', 'list-memories', 'edit-memory',
  'delete-memory', 'session-recover', 'smart-handoff', 'tool-health', 'env-info',
  'memory-usage', 'ai-config', 'ai-models', 'config', 'session-list', 'session-info',
  'get-shared-context', 'agent-list', 'agent-register', 'agent-history', 'invocation-log',
  'rl-reward-stats', 'rl-dopamine-log', 'sequence-recommend', 'loci-recall',
  'graph-query', 'graph-find-path', 'checkpoint-save', 'checkpoint-restore', 'checkpoint-diff',
]);
...
async function maybeAutoSave(sessionId: string | undefined, toolName: string): Promise<void> {
  ...
  try {
    ...
    const tool = await registryManager.findAsync('write-memory');
    if (tool) {
      const args = { name: 'active-session', content: snapshot, overwrite: true };
