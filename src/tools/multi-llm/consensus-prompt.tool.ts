...
const schema = z.object({
  ...
  agent: z
    ...
    .default('plan')
    .describe('Agent or Mind type for board'),
...
export const consensusPromptTool: UnifiedTool = {
  ...
  description:
    'CEO-and-Board decision pattern — board models respond in parallel, CEO synthesizes the final answer. ' +
    'Use when: critical decisions need multi-model validation (architecture, security, strategy). ' +
    'Stronger than multi-prompt because it synthesizes, not just compares.',
  zodSchema: schema,
  metadata: {
    category: 'multi-llm',
    ...
    aiRouting: 'external',
    examples: [
      { args: { prompt: 'What architecture should we use for a real-time chat app?', boardModels: ['o:gpt-4.1', 'a:claude-sonnet-4-6'], ceoModel: 'a:claude-sonnet-4-6', agent: 'plan' }, description: 'Get consensus from two models with CEO synthesis' },
    ],
