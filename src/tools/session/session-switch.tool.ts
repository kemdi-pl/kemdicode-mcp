...
export const sessionSwitchTool: UnifiedTool<typeof schema> = {
  name: 'session-switch',
  description: 'Switch session or update session model. Use when: changing the active session or updating the AI model mid-session.',
  zodSchema: schema,
  ...
  metadata: {
    ...
    examples: [
      ...
      { args: { sessionId: 'session-1', updateModel: 'gpt-4.1' }, description: 'Switch session and update its model' },
    ],
  ...
  execute: async (args) => {
    ...
    lines.push(
      ...
      '╚══════════════════════════════════════════════════════════════════╝',
      '',
      'Recommendation: Run `session-recover` to load cognition context for this session.',
    );
