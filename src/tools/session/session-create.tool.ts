...
export const sessionCreateTool: UnifiedTool<typeof schema> = {
  name: 'session-create',
  description: 'Create 1-10 sessions with AI models. Returns session IDs. Use when: starting new AI sessions for multi-agent or parallel workflows.',
  zodSchema: schema,
  ...
  metadata: {
    ...
    tags: ['session', 'create'],
    examples: [
      { args: { sessions: [{ model: 'gpt-4.1', cwd: '/opt/my-project' }] }, description: 'Create a single session with GPT-4.1' },
      { args: { sessions: [{ model: 'claude-sonnet-4-6', sessionId: 'backend-dev' }, { model: 'gpt-4.1', sessionId: 'frontend-dev' }] }, description: 'Create two sessions with different models' },
    ],
