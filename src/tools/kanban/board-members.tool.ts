...
const operationSchema = z.object({
  ...
  action: z.enum(['list', 'update-role', 'remove']).default('list').describe('Action: list (show members), update-role (change member role), remove (remove member from board)'),
  targetAgentId: z.string().optional().describe('Agent to modify (for update-role/remove)'),
  newRole: z
    .enum(['owner', 'admin', 'member', 'viewer'])
    .optional()
    .describe('New role (action=update-role): owner (full control), admin (manage members+tasks), member (create+edit tasks), viewer (read-only)'),
});
