...
export async function startHttpServer(host: string, port: number): Promise<Server> {
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    // Live orchestration monitoring endpoint
    // GET /orchestrations — list all active orchestrations
    // GET /orchestrations/:id — get specific orchestration status
    if (url.startsWith('/orchestrations') && req.method === 'GET') {
      const idMatch = url.match(/^\/orchestrations\/([^/]+)$/);
      if (idMatch) {
        const status = await getOrchestrationStatus(idMatch[1]);
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ active: all.length, orchestrations: all }));
      }
      finishRequest(200);
      return;
    }
  ...
  httpServer.listen(port, host, () => {
    ...
    Logger.info(`Orchestrations: http://${host}:${port}/orchestrations`);
